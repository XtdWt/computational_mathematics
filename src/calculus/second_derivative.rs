...
fn second_order_forward_difference(f: &Function, x: f64, h: f64) -> f64 {
    return (f(x + 2.0 * h) - 2.0 * f(x + h) + f(x)) / h.powi(2);
}


fn second_order_backward_difference(f: &Function, x: f64, h: f64) -> f64 {
    return (f(x) - 2.0 * f(x - h) + f(x - 2.0 * h)) / h.powi(2);
}


fn second_order_central_difference(f: &Function, x: f64, h: f64) -> f64 {
    return (f(x + h) - 2.0 * f(x) + f(x - h)) / (h.powi(2));
}


pub fn second_derivative(f: &Function, x: f64, h: f64, method: DerivativeType) -> f64 {
    return match method {
