...
>(
    ...
    for _ in 0..n_max {
        ...
        if constraint(x_next, f(x_next)) && f(x_next) <= f(x) {
...
use super::*;
    ...
    fn test_mcmc_method_quadratic_no_constraint() {
        ...
        let constraint = |_x: f64, _y: f64| true;
    ...
    fn test_mcmc_method_quadratic_constraint() {
        let f = |x: f64| (x - 3.0).powi(2);
        let df = |x: f64| 2.0 * (x - 3.0);
        let constraint = |x: f64, _y: f64| x < 2.5;
        ...
        // looser check due to probabilistic test
        assert_almost_eq!(result, 2.5, 1e-2);
    }

    #[test]
    fn test_mcmc_method_quadratic_x_and_y_constraint() {
        let f = |x: f64| (x - 3.0).powi(2);
        let df = |x: f64| 2.0 * (x - 3.0);
        let constraint = |x: f64, y: f64| y - x > -2.5;
        ...
        // looser check due to probabilistic test
        assert_almost_eq!(result, 2.63397, 1e-2);
    }
}
